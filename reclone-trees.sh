...
for m in "${DEVICES[@]}"; do
  ...
  git clone -j"$(nproc --all)" -b "$BRANCH" --single-branch "$URL/android_device_xiaomi_$m" "$(pwd)/device/xiaomi/$m"
  [ -d "$(pwd)/vendor/xiaomi/$m" ] && rm -rf "$(pwd)/vendor/xiaomi/$m"
  git clone -j"$(nproc --all)" -b "$BRANCH" --single-branch "$URL/proprietary_vendor_xiaomi_$m" "$(pwd)/vendor/xiaomi/$m"
done
